...
from mass_evictions.models import Docket, Filings, Plaintiffs, Defendants, Attorneys

dockets = Docket.objects.filter(text__contains='Attorney Appearance').annotate(
    text_len=Length('text')).filter(text_len__gt=len('Attorney Appearance'))
...
def run_extraction():
    ...
    with open("results.csv", "a+") as f:
        fieldnames = ["docket_id", "text", "action", "new_attorney_type", "new_attorney",
                      "new_attorney_bar", "p_name", "ptf_bar", "d_name", "def_bar",
                      "client"]
        ...
        for docket in dockets:
            ...
            if res == (None, None, None, None):
                ...
                filing = Filings.objects.get(docket_id=docket.docket_id)
                plaintiffs = Plaintiffs.objects.filter(
                    docket=docket.docket_id).values_list('name')
                defendants = Defendants.objects.filter(
                    docket=docket.docket_id).values_list('name')
                ...
                row = {"docket_id": docket.docket_id,
                       "text": docket.text,
                       "action": res[2],
                ...
                successes += 1
    return {"failures": failures, "successes": successes, "attorney_hits": attorney_hits, "attorney_exists": attorney_exists}
...
def get_attorney_and_client_from_str(input_str):
    ...
    try:
        ...
        attorney, action, client = re.split(
            r"(?:\s*)(added as| added for|withdrawn for|dismissed for|dismissed\/withdrawn for|dismissed\/withdrawn as|withdrawn as)(?:\s*)", parties)
        action = action.lower()
